...
class MKViewToImageViewController: UIViewController {
    ...
    func setSubViews() {
        if (self.textView == nil) {
            self.textView = UITextView.init(frame: CGRect.init(x: 40, y: 200, width: UIScreen.main.bounds.width - 80, height: 60))
        ...
        if self.savePic == nil {
            self.savePic = UIButton.init(frame: CGRect.init(x: 40, y: self.textView.frame.maxY + 40, width: UIScreen.main.bounds.width - 80, height: 60))
...
//            self.savePic.setBackgroundImage(UIIma, for: <#T##UIControl.State#>)
        ...
        if self.imageView == nil {
            self.imageView = UIImageView.init(frame: CGRect.init(x: 40, y: self.savePic.frame.maxY + 40, width: UIScreen.main.bounds.width - 80, height: 200))
